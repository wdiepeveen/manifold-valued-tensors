...
function decoupled_curvature_corrected_low_rank_approximation(M, q, X, rank; stepsize=1/100, max_iter=200, change_tol=1e-6)
    n = size(X)[1]
    ...
    Rr = Euclidean(r)
    CCL = [(MM, p) -> curvature_corrected_loss(M, q, X, R_q, U[i,:]) for i in 1:n]
    ...
    # do GD routine 
    Ξ = gradient_descent(Rr, CCL, gradCCL, ProductRepr(U, Σ, V); stepsize=ConstantStepsize(stepsize),
