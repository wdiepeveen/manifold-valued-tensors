...
function stochastic_curvature_corrected_low_rank_approximation(M, q, X, rank; stepsize=1/100, max_iter=200, change_tol=1e-6)
    n = size(X)[1]
    d = manifold_dimension(M)
    r = min(n, d, rank)
    ...
    gradCCL = [(MM, p) -> sum([gradient_curvature_corrected_loss(M, q, X, submanifold_component(p, 1), submanifold_component(p, 2), submanifold_component(p, 3), i, k) for i in 1:n]) for k in 1:d]

    # do GD routine 
    @time Ξ = stochastic_gradient_descent(N, gradCCL, ProductRepr(U, Σ, V); stepsize=ConstantStepsize(stepsize),
